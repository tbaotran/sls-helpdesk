          ...
          <aside className="w-96 bg-white border-l border-[#D2BA92] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="p-8 space-y-6 overflow-y-auto flex-1">
              <div>
                ...
                <h1 className="text-2xl font-serif font-bold text-[#2E2D29] leading-tight mt-3">{selectedTicket.title}</h1>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-[#4D4F53] uppercase tracking-widest">Description</label>
                <p className="text-sm text-[#2E2D29] leading-relaxed bg-[#F4F4F4] p-4 rounded-xl italic border border-[#D2BA92]/30">"{selectedTicket.description || 'No description provided.'}"</p>
              </div>
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                <div className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Status<p className={`mt-1 text-sm capitalize font-bold ${selectedTicket.status === 'resolved' ? 'text-green-600' : 'text-[#8C1515]'}`}>{selectedTicket.status}</p></div>
              ...
              <div className="pt-6 border-t border-gray-100">
                ...
                <div className="space-y-5">
                  {activities.length === 0 ? <p className="text-xs text-gray-400 italic">No activity recorded yet.</p> : activities.map((log) => (
